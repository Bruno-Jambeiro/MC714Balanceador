...
\begin{figure}[H]
\centerline{\includegraphics[width=0.8\columnwidth]{../plots/comparacao_politicas_ER.png}}
\caption{Comparação de $E[R]$: Analítico x Simulado.}
\label{fig:comparacao}
\end{figure}

A Figura~\ref{fig:temporal_estavel} ilustra a evolução temporal de $N(t)$, $R(t)$ e $\bar{U}(t)$ nos extremos do regime estável, contrastando o comportamento de baixa carga ($\lambda=0{,}6$, $\rho=0{,}2$) com o de alta carga ($\lambda=2{,}7$, $\rho=0{,}9$). Em ambos os casos, o sistema converge para o regime estacionário após o período de \textit{warm-up}, porém com níveis de ocupação e variabilidade radicalmente distintos.

\begin{figure}[H]
\centering
\begin{minipage}[b]{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{../plots/simulacao_lambda_0.6.png}
    \small (a) $\lambda = 0{,}6$
\end{minipage}
\hfill
\begin{minipage}[b]{0.48\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{../plots/simulacao_lambda_2.7.png}
    \small (b) $\lambda = 2{,}7$
\end{minipage}
\caption{Evolução temporal de $N(t)$, $R(t)$ e $\bar{U}(t)$: baixa carga vs.\ alta carga estável.}
\label{fig:temporal_estavel}
\end{figure}
...
\begin{figure}[H]
\centerline{\includegraphics[width=0.8\columnwidth]{../plots/verificacao_lei_de_little.png}}
...
A Figura~\ref{fig:instavel} ilustra a evolução temporal de $N(t)$, $R(t)$ e $\bar{U}(t)$ para as três políticas no regime instável. Nota-se o crescimento secular e contínuo das filas em todas as políticas, com a utilização média dos servidores se aproximando de $100\%$.

\begin{figure}[H]
\centerline{\includegraphics[width=\columnwidth]{../plots/simulacao_lambda_3.3_instavel.png}}
\caption{Simulação temporal no regime instável ($\lambda = 3{,}3$, $3\mu = 3{,}0$): crescimento ilimitado de $N(t)$ e $R(t)$.}
\label{fig:instavel}
\end{figure}

A Figura~\ref{fig:fluido} compara o crescimento de $N(t)$ observado nas simulações com o modelo fluido teórico, confirmando a taxa de acúmulo de $0{,}3$ requisições por unidade de tempo prevista analiticamente.

\begin{figure}[H]
\centerline{\includegraphics[width=\columnwidth]{../plots/modelo_fluido_lambda_3.3.png}}
\caption{Verificação do modelo fluido: $N(t) \approx (\lambda - 3\mu)\,t = 0{,}3t$ no regime instável.}
\label{fig:fluido}
\end{figure}
...
A Tabela~\ref{tab:divisao} apresenta a divisão das responsabilidades entre os autores ao longo do desenvolvimento do projeto.

\begin{table}[H]
\caption{Divisão de Tarefas entre os Autores}
\label{tab:divisao}
\centering
\begin{tabular}{|p{0.52\columnwidth}|p{0.38\columnwidth}|}
\hline
\textbf{Tarefa} & \textbf{Responsável} \\
\hline
Modelagem analítica dos modelos de filas ($M/M/1$, $M/M/c$) & Lucas Bussinger \\
\hline
Derivação formal das métricas de desempenho ($E[N]$, $E[R]$, $E[T_Q]$) & Lucas Bussinger \\
\hline
Prova de independência dos servidores sob política aleatória & Lucas Bussinger \\
\hline
Análise analítica de estabilidade e modelo fluido & Lucas Bussinger \\
\hline
Implementação da simulação em Python (\texttt{classes.py}, \texttt{simulacoes.py}) & Bruno Jambeiro \\
Implementação do motor de simulação e estruturas de dados (\texttt{classes.py}: \texttt{Pacote}, \texttt{Servidor}, \texttt{Balanceador}, \texttt{Simulador}) & Lucas Bussinger \\
\hline
Implementação das políticas de roteamento no \texttt{Balanceador} (Aleatória, Round-Robin, Menor Fila) & Lucas Bussinger \\
\hline
Orquestração dos experimentos, réplicas e cálculo de intervalos de confiança (\texttt{simulacoes.py}) & Bruno Jambeiro \\
\hline
Geração e formatação dos gráficos (\texttt{graficos.py}) & Bruno Jambeiro \\
\hline
Configuração do repositório e ambiente de execução & Bruno Jambeiro \\
\hline
Interpretação conjunta dos resultados numéricos & Ambos \\
\hline
Redação do relatório & Ambos \\
\hline
\end{tabular}
\end{table}
